...
/* Edited By Eric Budiman Gosno <eric.gosno@gmail.com>
 * Last modified : 04/25/2013
 * Fixed Bugged Regular Expression at step 1b & 1c
 * /
...
namespace Stemmer
{
    public class PorterStemmer
    {
        ...
        #region Exception
        public KeyValuePair<bool,string> IsException1(string p_words)
        {
            switch (p_words)
            {
                case "skis":
                    p_words = "ski";
                    return new KeyValuePair<bool,string>(true,p_words);
                case "skies":
                    p_words = "sky";
                    return new KeyValuePair<bool, string>(true, p_words);
                case "dying":
            ...
            return new KeyValuePair<bool, string>(false, p_words);
        }
        #endregion
     ...
     */
        private string Step1(string p_word)
        {
            ...
            if (p_word.Substring(p_word.Length - 1) == "s")
            {
                ...
                else if (p_word.Length > 2 && p_word.Substring(p_word.Length - 1) == "s" & p_word.Substring(p_word.Length - 2, 1) != "s")
                {
            ...
            {
                //if (Regex.IsMatch("/([aeiou]|[^aeiou]y).*(ed|ing)$/", p_word)) --> this regular expression bugged
                regexMatch = false;
                if (p_word.Length > 2 && p_word.Substring(p_word.Length - 2) == "ed")
                {
                    for (int i = 0; i < p_word.Length-2; i++)
                    {
                        if (!is_consonant(p_word, i)) regexMatch = true;
                    }
                }
                else if (p_word.Length > 3 && p_word.Substring(p_word.Length - 3) == "ing")
                {
                    for (int i = 0; i < p_word.Length - 3; i++)
                    {
                        if (!is_consonant(p_word, i)) regexMatch = true;
                    } 
                }
                ...
                { // vowel in stem
                    ...
                    else if(p_word.Length > 2)
                    {
                        ...
                        if (this.is_consonant(p_word, p_word.Length-1) &&
            ...
            //if (Regex.IsMatch("/([aeiou]|[^aeiou]y).*y$/", p_word)) --> this regular expression bugged
            regexMatch = false;
            if (p_word.Length > 1 && p_word.Substring(p_word.Length - 1) == "y")
            {
                for (int i = 0; i < p_word.Length - 2; i++)
                {
                    if (!is_consonant(p_word, i)) regexMatch = true;
        ...
        {
            if (p_word.Length <= 2) return p_word;
        ...
        {
            if (p_word.Length <= 1) return p_word;
     ...
     */
        private bool is_consonant(string p_word, int p_pos)
        {
            ...
            if (p_word.Length <= p_pos) return false;
            ...
            switch (charpos)
            {
                ...
                case "y":
                    ...
                    {
                        if (this.is_consonant(p_word, p_pos - 1)) return false;
                        else return true;
                    }
                default:
                    return true;
        ...
        private bool _o(string p_word)
        {
            if (p_word.Length >= 3)
            {
                if (this.is_consonant(p_word, p_word.Length-1) && !this.is_consonant(p_word, p_word.Length -2) &&
                     this.is_consonant(p_word, p_word.Length-3))
                {
